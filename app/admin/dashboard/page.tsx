...
// ── Image Upload Zone ─────────────────────────────────────────────────────────
function ImageUploadZone({ value, onChange }: { value: string; onChange: (base64: string) => void }) {
      ...
      {value ? (
        ...
        >
          {loading ? (
            ...
            <>
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ backgroundColor: 'rgba(196,120,138,0.12)' }}>
                <ImageIcon size={26} style={{ color: '#C4788A' }} />
              </div>
              <div>
                <p className="text-sm font-semibold" style={{ color: '#8B4E6B' }}>Drag &amp; drop your photo here</p>
                <p className="text-xs text-gray-400 mt-1">or <span className="underline font-medium" style={{ color: '#C4788A' }}>click to browse</span></p>
              </div>
        ...
        {activeTab === 'products' && (
          <div className="p-8">
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden border" style={{ borderColor: 'rgba(196,120,138,0.15)' }}>
              {loadingProducts ? (
                <div className="py-20 flex items-center justify-center gap-3" style={{ color: '#C4788A' }}>
